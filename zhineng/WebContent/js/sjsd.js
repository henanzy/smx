...
function getDate(date){
	if(date!=null&&date!=""&&date.length>9){
		var year = date.substring(0,4);
		var mm = date.substring(4,6);
		var dd = date.substring(6,8);
		var hh = date.substring(8,10);
		return year+"-"+mm+"-"+dd+" "+hh+"时";
	}else{
		return " ";
	}
}
$(document).ready(function(){
	...
	function jsArrChange(json){
		for (var i = 0 ; i < json.length ; i ++) {
			...
			arr1[2] = getDate(json[i].date1+"");
			arr1[3] = getDate(json[i].date2+"")
			arr1[4] = getDate(json[i].date3+"")
			arr1[5] = getDate(json[i].date4+"")
			arr1[6] = getDate(json[i].date5+"")
			arr1[7] = getDate(json[i].date6+"")
			arr1[8] = getDate(json[i].date7+"")
			arr1[9] = getDate(json[i].date8+"")
			qgxxList.push(arr1);
		};
	}
	jsArrChange(list);
...
$("#search_btn").click(function(){
	...
	var compareWordList = [];
		...
		compareWord(xq,ld,compareWordList);
...
function compareWord(xq,ld,compareWordList){
