...
// 表格
	...
	function jsArrChange(json){
		for (var i = 0 ; i < json.length ; i ++) {
			...
			arr1[10] = json[i].CjqTime;
			arr1[11] = json[i].RecordTime;
			arr1[12] = json[i].IDNum;
			arr1[13] = json[i].ValAd;
			arr1[14] = json[i].WCAd;
			arr1[15] = json[i].CjqWz;
			arr1[16] = json[i].SFJF;
			arr1[17] = json[i].Telephone;
	...
	$("#search_btn").click(function(){
		...
		compareWord(xq,ld,dy,hh,startTime,endTime,compareWordList);
    ...
    function tableToExcel(){
        ...
        '<td>卡号</td><td>传感器地址</td><td>安装位置</td>'+
        '<td>缴费状态</td><td>电话号码</td>'
        '<td>面积</td><td>区管ID</td>'+
        '</tr>';
...
function compareWord(xq,ld,dy,hh,startTime,endTime,compareWordList){
	var json;
	compareWordList.length=0;
	
	$.ajax({
		url : getRootPath()+"/yhInfo/SeaHistory.action", 
		async : false,
		dataType : "json",
		data : {
			"xqName":xq,
