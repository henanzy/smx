...
$(document).ready(function(){
	...
	function jsArrChange(json){
		for (var i = 0 ; i < json.length ; i ++) {
			...
			arr1[17] = json[i].IDNum
			arr1[18] = json[i].HTemSet;
			arr1[19] = json[i].MTemSet;
			arr1[20] = json[i].LTemSet;
			arr1[21] = json[i].QgID;
		...
		$("#change_zdjk").click(function(){
			 $('input[name="check"]:checked').each(function (index, item) {
				 ...
				 var valad=td[15].innerHTML;
				          ...
				          $.ajax({
			                     type: "post",
			                    url: getRootPath()+"/sbglCon/updateyhlb.action",
			                      dataType:'json',
			                      async:false,
			                  	data:{	
			      					"valAd":valad,
...
$("#sel_zdjk").click(function(){
		
		$.ajax({
			url : getRootPath()+"/sbglCon/findzdjk.action", 
			async : false,
			dataType : "json",
			data : {
				"yhlb":$("#yhlbs").val(),
			...
			success : function(data) {
			 json=data.yhlb;
				...
				tbodydis(shebList,compareWordList,1);
			}
		});
		...
		for(var i = 0; i < newlist.length; i++) {
			if(i >= startRow && i <= endRow) { // 通过间隔分隔数组
				...
				for (var j = 0 ; j <newlist[i].length ; j ++) {
    				...
    				}else if(newlist[i][23]=='重点监控' ){
    					html += "<td style='color:red'>" + newlist[i][j] + "</td>";
    				} else if(newlist[i][23]=='退费停暖'){
    					html += "<td style='color:#FF8C00'>" + newlist[i][j] + "</td>";
    				}else{
