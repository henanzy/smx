...
//  表格
    ...
    function tableToExcel(){
      ...
      $("#cxqgtx").click(function(){
 		 ...
 		 $('input[name="check"]:checked').each(function (index, item) {
 			 ...
 			 var qgid=td[2].innerHTML;
 			          $.ajax({
 		                     type: "post",
 		                    url: getRootPath()+"/qg/QgTS.action",
 		                      dataType:'json',
 		                      async:false,
 		                  	data:{	
 		      					"qgId":qgid,
      ...
      $("#cxskqtx").click(function(){
    	
  		 $('input[name="check"]:checked').each(function (index, item) {
  			 var row=$(this).parent("td").parent("tr");
  			 var td = row.find("td");
  			 var qgid=td[2].innerHTML;	
  			 var skqSbh=td[8].innerHTML;
  			          $.ajax({
  		                     type: "post",
  		                    url: getRootPath()+"/qg/SkqState.action",
  		                      dataType:'json',
  		                      async:false,
  		                  	data:{	
  		      					"qgid":qgid,
      ...
      $("#cxjzqtx").click(function(){
      	
   		 $('input[name="check"]:checked').each(function (index, item) {
   			 var row=$(this).parent("td").parent("tr");
   			 var td = row.find("td");
   			 var qgid=td[2].innerHTML;	
   			 var jzqId=td[3].innerHTML;			 			 
   			          $.ajax({
   		                     type: "post",
   		                    url: getRootPath()+"/qg/jzqState.action",
   		                      dataType:'json',
   		                      async:false,
   		                  	data:{	
   		      					"qgid":qgid,
  	...
  	$("#FsCs").click(function(){
  		 ...
  		 $('input[name="check"]:checked').each(function (index, item) {
  			 ...
  			 var qgid=td[2].innerHTML;			 			 
  			          $.ajax({
  		                     type: "post",
  		                    url: getRootPath()+"/qg/FsCs.action",
  		                      dataType:'json',
  		                      async:false,
  		                  	data:{	
  		      					"wdsd":$("#wdsd").val(),
