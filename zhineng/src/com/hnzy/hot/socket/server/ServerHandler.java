	...
	public void DkQ(byte[] base, IoSession session, Object message, String clientIp, Connection connc) {
		try {
			...
			if (start.equals("F0") && end.equals("FF") && je.equals("" + jy + "")) {
				...
				String Oraclesql = "select distinct YHBM,JFZT   from sf_jmxx_v where   CNQ='2019-2020' and JFZT=0 and YHKH='"
						+ khId + "'";
				// 连接oracle数据库
				...
				if (YHBM != null) {
					...
					logs.info("卡号----------：" + khId);
					// 查找阀门是否开关
					String SqlSelect = "select distinct fm.Status,fm.QgID,fm.RoomTemp,yh.Skfl,yh.Yhfl,yh.ValAd,yh.SFJF,fm.LockSt from T_YhInfo yh,T_FmInfo fm where yh.ValAd=fm.ValAd and yh.YHBM='"
							+ YHBM + "'";
					// 执行查询sql
					...
					while (rstS.next()) {
						...
						LockSt=rstS.getString("LockSt");
						Skfl=rstS.getString("Skfl");
//					}//多个阀门可循环
					if (valAd != null) {
						...
						// 读阀门是否成功
						dfStatus = MapUtilsSkq.getMapUtils().get("df");//阀门地址
						fmID = MapUtilsSkq.getMapUtils().get("fmID");//十六进制阀门地址
						// 如果读阀门失败
						if (dfStatus == null) {
							// 发送读阀门指令
							SJtring = "F00F0400" + qgID + "040" + fmd;
						...
						if (JFZT == 0) {
							//查询阀门是否锁定
							if(null!=Skfl&&Skfl.equals("0")&&LockSt.equals("是")&&!Yhfl.equals("重点监控")&&!Yhfl.equals("退费停暖")){
								
								//发送自动运行
								SJtring = "F0160900" + qgID + "040" + fmd + "01030001FFFFFF";	
								sessionmap = cz(SJtring, clientIp);
								try {
									Thread.sleep(1000);
								} catch (InterruptedException e) {
								...
								if(sessionmap==false){
									...
									if(sessionmap==false){
										...
										//自动运行失败
										String jaString = "F00F3B00" + DkqId + "" + KhId + "05";
										...
										//阀门已自动运行
										String jaString = "F00F3B00" + DkqId + "" + KhId + "04";
										sessionmap = cz(jaString, clientIp);
								...
								}else if(null!=Skfl&&Skfl.equals("1")){
							
									//发送自动运行
//									SJtring = "F0160900" + qgID + "040" + fmd + "01030001FFFFFF";	
									SJtring = "F0160900" + qgID + "040" + fmd + "01FF0101FFFFFF";
